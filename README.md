...
копирует свежий код на сервер и перезапускает systemd-сервис.
...
2. Убедитесь, что на сервере уже существует каталог `/root/LeadForcePython`
   и в нём инициализирован git-репозиторий проекта (`git clone ...`).
3. При пуше в ветку `main` либо при ручном запуске workflow (кнопка
   **Run workflow**) выполнится копирование файлов через `scp`, установка
   зависимостей (`pip install -r requirements.txt`) и перезапуск сервиса
   `leadforce`.
...
Скрипт обновит репозиторий (жёсткий `git reset --hard` на актуальную ветку),
переустановит зависимости и перезапустит сервис.
